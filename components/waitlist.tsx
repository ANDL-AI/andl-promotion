...
const Waitlist: React.FC = () => {
  ...
  useEffect(() => {
    const fetchEnrolledCount = async () => {
      try {
        const sheetUrl =
          "https://docs.google.com/spreadsheets/d/1Df_ySvndRqOWlBVcNxocSGaXU8vWolWZoc4Wyfn1wtA/gviz/tq?tqx=out:csv&sheet=Sheet1";
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    const googleFormUrl =
      "https://docs.google.com/forms/d/e/1FAIpQLScRkwY198g2ho75hz1RMYRBKHVBRn4c39VWMd3Ir9hZokzw8g/formResponse";
    ...
    try {
      ...
      // Success message
      message.success("Thank you for joining the waitlist! We'll contact you soon.");
    } catch (error) {
    ...
    <section id="waitlist" className="px-4 pb-16 pt-2 bg-[#f9f9f9]">
      ...
      {enrolledCount !== null && (
        <p className="text-lg text-center mb-4 font-semibold">
          <span className="font-semibold text-[#6321E6]">{enrolledCount}</span>{" "}
          people have already joined the waitlist!
        </p>
      )}
      <p className="text-center mb-8 max-w-2xl mx-auto">
        Be the first to experience the future of education. Sign up for our waitlist and get early
        access to ANDL.
      </p>
